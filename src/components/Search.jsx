...
import { AuthProvider, userAuth } from "../features/AuthContext";
import { userService } from "../services/userService";

const Search = ({ optionsData, userData }) => {
  const [showOptions, setShowOptions] = useState(false);
  const [search, setSearch] = useState("");
  const [selectedOptions, setSelectedOptions] = useState(optionsData[0]);
  const { searchByName } = userAuth();
  ...
  return (
    <form class="max-w-lg mx-auto my-3" onSubmit={(e) => handleSearch(e)}>
      <div class="flex">
        <label
          for="search-dropdown"
        ...
        {showOptions && (
          <div
            id="dropdown"
            class="absolute  z-9999bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700"
          >
            <ul
